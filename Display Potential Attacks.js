...
function isOnLine (sourcePlanetId, targetPlanetId, mouseX, mouseY) {
  ...
  //this is the real deal down below. you might not want to touch this part
    ...
    console.log("second test case results: " + (Math.ceil(mouseY) >= Math.ceil(mouseY) >= Math.ceil(mouseX * slope + bValue - (viewport.canvasToWorldDist(LINE_WIDTH * LINE_WIDTH_ERROR_FACTOR) + MIN_LINE_WIDTH_SIZE) * Math.abs(Math.atan(slope) / Math.PI))));
    console.log("is " + Math.ceil(mouseY) + " >= " + Math.ceil(mouseX * slope + bValue - (viewport.canvasToWorldDist(LINE_WIDTH * LINE_WIDTH_ERROR_FACTOR) + MIN_LINE_WIDTH_SIZE) * Math.abs(Math.atan(slope) / Math.PI)) + "?");
    console.log("third test case results: " + (Math.ceil(mouseX) <= Math.ceil(mouseX) <= Math.ceil(mouseY * inverseSlope + inverseBValue + (viewport.canvasToWorldDist(LINE_WIDTH * LINE_WIDTH_ERROR_FACTOR) + MIN_LINE_WIDTH_SIZE) * Math.abs(Math.atan(inverseSlope) / Math.PI))));
